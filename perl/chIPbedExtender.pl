...
#This scripts extends the length of the fragments in your bed file to the number you enter in third argument
...
#$pointbedname =   $pointbedname.".extended.bed";

my $repairedbedname = $pointbedname;
$repairedbedname =~ s/\.bed$/\.repaired.bed/;
#$repairedbedname = $repairedbedname.".repaired.bed";

print "extending the fragments in your bed file\n";

my $bedtopoint = "cat $bedfilename".' | awk \'{if($6 == "+"){print $1 "\t" $2 "\t" $2+'.$fragmentsize.' "\t*\t" $5 "\t" $6}else{print $1 "\t" $3-'.$fragmentsize.' "\t" $3 "\t*\t" $5 "\t" $6}}\' > '.$pointbedname;
`$bedtopoint`;
