...
# USAGE : Rscript ~/src/bed2cvg.R sample.bed control.bed 2> cov.log
...
suppressPackageStartupMessages(library('BSgenome.Mmusculus.UCSC.mm9','ShortRead'))
suppressPackageStartupMessages(library(ff))
suppressPackageStartupMessages(library(chipseq))
...
# setting parameters
fragment.size=250;chr.size<-seqlengths(Mmusculus)
...
#Removing duplicates for the same start positions and same strand orientation - DATA SUPPLIED IS ALREADY CLEANED
...
sizeChip<-length(chip.GRanges)
sizeControl<-length(control.GRanges)
smallest.size<-min(sizeControl,sizeChip)

print ("Sampling Datasets")

# normalizing chip and control library size
chip.norm.aln<-sample(chip.GRanges,smallest.size,replace=FALSE)
control.norm.aln<-sample(control.GRanges,smallest.size,replace=FALSE)
